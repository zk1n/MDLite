...
更新日: 2026-09-20
対象Task: `20260920-mdlite-acceptance-hardening` Revision 1

この表は実装、自動試験、実GUI経路試験、Human確認、外部準備待ちを分離する。
`実装済み`や`自動PASS`はHumanの操作感・見た目の受入を意味しない。性能値と
R-01〜R-08の根拠は [acceptance-hardening-report.md](acceptance-hardening-report.md) に記録する。
...
| A-02 | 表示更新はsourceを書き換えず、アプリ管理source履歴だけをUndo/Redoとして公開。表示専用native Undoは通知抑止中に破棄。同一行で識別不能な複数画像はraw Markdownへfail-closedし、完全にcollapseしたsource rangeだけnative描画 | Core byte/Dirty・複数画像identity・隣接画像回帰、Debug/Release GUI Save・Undo/Redo・隣接raw保存経路 | 自動PASS |
| A-03 | IME状態を文書単位で保持し、合成中のsync／保存／表操作を停止 | compactを含むnotification経路を回帰。Microsoft IME／ATOK実変換は未実施 | 実装済み・Human gate |
| A-04 | 見出し・本文・native image object・表presentationを同一RichEditへ統合 | parser/source-view回帰。実画像と表の自然な見え方はHuman未確認 | 実装済み・Human gate |
...
| A-21 | 本体にnetwork APIなし。休日は同梱 | 追跡依存／文字列scanとRelease GUI通常操作中TCP endpoint 0。OS shell/外部CLIは明示操作のみ | 自動PASS（OS全体packet captureは未実施） |
| A-22 | ReleaseのP0〜P5測定scriptとJSONを作成し実行 | 通常1〜6文書はpeak WS最大44,343,296 bytes、入力p95最大0.701ms。無音P5 100/100、失敗0 | 50,000,000-byte目標PASS |
...
| A-27 | 同一RichEdit上のcell grid、表source transaction、Tab/Shift+Tab、矢印境界、行列操作、Undo接続 | grid cell解析、EOF/CRLF/escaped pipe/code pipe、GUI Undo/Redo回帰 | 自動PASS・hit-testの操作感はHuman評価待ち |
...
| A-30 | 日曜始まり、同梱日本休日、既存Dairy、tooltip | 2025〜2027 dataset回帰。DPIごとの見え方はHuman未確認 | 自動PASS・Human gate |
| A-31 | PNG/JPEG/GIF/WebP/SVG経路、clipboard、D&D、比率固定resize、SVG XML安全判定、画像ごとのanimation期限 | 5形式のnative RichEdit挿入、GIF partial frame/offset/transparency/disposal 2/3、GIF/WebP異周期timing、SVG同一bytes検査、複数object前後source保存、隣接raw保存、Release P4 | 自動PASS・見え方はHuman gate |
