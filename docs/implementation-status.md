...
| A-30 | 日曜始まり、同梱日本休日、検証付きローカルCSV追加、common許可時の月次cache、既存Dairy、tooltip | 同梱データとCSV import、fake clock、mock HTTP 200/304/404/500/timeout/HTML/大幅減少、cache last-known-goodのpolicy回帰、最新 `gui-acceptance-settings-final21.json` のcalendar表示／非表示経路。DPIごとの見え方はHuman未確認 | 自動・mock PASS・Human gate（実HTTPはWinHTTP 12185でBLOCKED） |
