...
| native screenshot | PASS (evidence only) | 最新 `build/verification/native-screenshot-current-20260921.json` と `native-screenshot-current-20260921.png`。現行Release `MDLite.exe`（SHA-256 `132bc8628e50cc7cf2efb5be7bbc11cc3557de1bf7d9b83b476ee47977ce849c`）を架空fixtureで1280x800に起動し、PrintWindowで取得。Computer UseはMDLite window列挙後にWindowsロック画面で停止したため、操作・主観受入とは分離 |
| native screenshot before/after | EVIDENCE | 修正前 `native-screenshot-final.json` / `.png`（Release hash `833C975470D9AA5A0B0439E6CAADA830E8883FCB9A9BC92379964197C45F583F`）、旧修正後 `native-screenshot-responsive-final9.json` / `.png`（Release hash `495EBE4CEFFFE1C3E94F83065288D5D99CE291F088A7FBA62E70DCFD9FEBC7C9`）に加え、現行Releaseのfresh `native-screenshot-current-20260921.json` / `.png`（hash `132bc8628e50cc7cf2efb5be7bbc11cc3557de1bf7d9b83b476ee47977ce849c`）を取得。before/after画像は実native描画の比較証拠であり、Human視認性PASSではない |
