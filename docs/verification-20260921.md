...
| Fresh bounded quick performance | PASS | `build/verification/performance-release-table-geometry-quick-final6.json`。P1 six-document 29,425,664 bytes、P4 42,934,272 bytes、compact 3/3、P5 20/20 failures 0、入力p95最大2.852ms。現行Release exe SHA-256は `9922bb78dcbb63b63af0d3f3a58b25701c5b2c6b874ab965e983882968a2f87c`。 |
...
| native座標・CRLF/LF・画像・表 transaction | PASS | CoreTestsのnative snapshot/transaction回帰。表描画は可視行全体で測った共有列境界を使用 |
| native screenshot | PASS (evidence only) | `build/verification/native-screenshot-table-geometry-final6.json` と `native-screenshot-table-geometry-final6.png`。隔離fixtureのRelease `MDLite.exe`（SHA-256 `9922BB78DCBB63B63AF0D3F3A58B25701C5B2C6B874AB965E983882968A2F87C`）を1280x800でPrintWindow取得。CUA列挙不可のため操作・主観受入とは分離 |
...
| GUI acceptance script | PASS | `build/verification/gui-acceptance-table-geometry-final6.json`、pass=true。Release exe hash `9922bb78dcbb63b63af0d3f3a58b25701c5b2c6b874ab965e983882968a2f87c`。compact edit/Undo/Redo、Find/Replace、画像object隣接編集、recovery、TCP観測を実native UI経路で確認 |
| native screenshot / CUA操作 | PARTIAL | PrintWindowによる現行Releaseの実native画面は取得済み（`native-screenshot-table-geometry-final6.json`）。CUAにはnative appが列挙されず、クリック・ドラッグ・Humanの視認性は未確認 |
...
| QA-04 | table parser、可視行横断の共有column geometry、GUI編集経路 | 実DPIのcell hit-test/操作感は未実施 |
