...
| 設定・profile・picker native form | PASS (automated native UI path) | 最新 `build/verification/gui-acceptance-settings-final21.json`。実native modal `MDLite.NativeFormWindow` の設定Apply/Cancelとprofile Apply/Cancel経路、`MDLite.NativePickerWindow` のQuick Open／command palette filter・候補選択を確認。設定Cancel・profile CancelはfixtureのSHA-256を保持し、profile Applyはsilent試験の既存確認境界で保存確認を拒否してfixtureを保持したうえでform Apply・validation経路を通した。runtime_environmentにOS build、`RICHEDIT50W`/Msftedit.dll、DPI、window/client、font設定、caret、source/settings hashを記録した。workspace/outline tree、outline pane折畳み／復元、calendar表示／非表示も確認した。保存は既存のSettings/Profile validatorへ通す |
| native screenshot | PASS (evidence only) | 最新 `build/verification/native-screenshot-current-20260921.json` と `native-screenshot-current-20260921.png`。現行Release `MDLite.exe`（SHA-256 `ad2e5bfbca40ff60d84594ab138ed76eee0fb552b5aecc7ede520ab317a5b767`）を架空fixtureで1280x800に起動し、RichEdit本文のロード（124文字）を確認後、前面化・再描画してPrintWindowで取得。画像SHA-256は `9a8d648242729ac94c31e7e243e45b3622bca2de67e705182fa3bd4a18e610db`。Computer UseはMDLite window列挙後にWindowsロック画面で停止したため、操作・主観受入とは分離 |
| native screenshot before/after | EVIDENCE | 修正前 `native-screenshot-final.json` / `.png`（Release hash `833C975470D9AA5A0B0439E6CAADA830E8883FCB9A9BC92379964197C45F583F`）、旧修正後 `native-screenshot-responsive-final9.json` / `.png`（Release hash `495EBE4CEFFFE1C3E94F83065288D5D99CE291F088A7FBA62E70DCFD9FEBC7C9`）に加え、現行Releaseのfresh `native-screenshot-current-20260921.json` / `.png`（exe hash `ad2e5bfbca40ff60d84594ab138ed76eee0fb552b5aecc7ede520ab317a5b767`、画像hash `9a8d648242729ac94c31e7e243e45b3622bca2de67e705182fa3bd4a18e610db`）を取得。before/after画像は実native描画の比較証拠であり、Human視認性PASSではない |
...
| GUI acceptance script | PASS | 最新 `build/verification/gui-acceptance-settings-final21.json`、pass=true。Release exe hash `ad2e5bfbca40ff60d84594ab138ed76eee0fb552b5aecc7ede520ab317a5b767`。compact edit/Undo/Redo、Find/Replace、workspace/outline tree、outline pane折畳み／復元、calendar表示／非表示、Quick Open／command paletteのfilter・候補選択、設定native form Apply/Cancel、profile native form Apply/Cancel、Cancel／Apply拒否時のfixture不変、runtime OS/RichEdit/DPI/window/client/font/source/settings metadata、画像object隣接編集、recovery、TCP観測を実native UI経路で確認 |
| native screenshot / CUA操作 | PARTIAL / BLOCKED | PrintWindowによる現行Releaseの実native画面は取得済み（`native-screenshot-current-20260921.json`）。Computer UseではMDLiteの実window列挙まではできたが、画面取得がWindowsロック画面となり前面化に失敗したため、クリック・ドラッグ・Humanの視認性は未確認 |
...
|---|---|---|
| QA-01 | GUI acceptanceのsource保存・recovery、Debug/Release build、runtime OS/RichEdit/DPI/window/client/font/source/settings metadata、修正前後と現行Releaseのnative PrintWindow screenshot（`native-screenshot-current-20260921.json`） | クリック操作・主観的な文字位置は未実施 |
...
| QA-07 | GUI acceptanceの検索/置換/compact/recovery、workspace/outline tree、outline pane折畳み／復元、calendar表示／非表示、Quick Open／command paletteのfilter・候補選択、Core calendar/settings、設定Apply/Cancelとprofile Apply/Cancelのnative form、Cancel／Apply拒否時のfixture不変 | outline/calendar click・drag、calendarのHuman操作とIME/視認性は未実施 |
...
| QA-10 | 301 checks、Release CTest、silent P5 100/100、設定/profile native form Apply/Cancel、Cancel／Apply拒否時のfixture不変、現行Release native screenshot evidence | ATOK・Human通知/視認性は未実施 |
