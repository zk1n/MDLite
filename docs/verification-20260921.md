...
| 設定・profile native form | PASS (automated native UI path) | 最新 `build/verification/gui-acceptance-settings-final15.json`。実native modal `MDLite.NativeFormWindow` の設定Apply/Cancelとprofile Apply/Cancel経路を確認。設定Cancel・profile CancelはfixtureのSHA-256を保持し、profile Applyはsilent試験の既存確認境界で保存確認を拒否してfixtureを保持したうえでform Apply・validation経路を通した。連続Promptを通常編集経路から除去し、保存は既存のSettings/Profile validatorへ通す |
...
| GUI acceptance script | PASS | 最新 `build/verification/gui-acceptance-settings-final15.json`、pass=true。Release exe hash `132bc8628e50cc7cf2efb5be7bbc11cc3557de1bf7d9b83b476ee47977ce849c`。compact edit/Undo/Redo、Find/Replace、設定native form Apply/Cancel、profile native form Apply/Cancel、Cancel／Apply拒否時のfixture不変、画像object隣接編集、recovery、TCP観測を実native UI経路で確認 |
...
|---|---|---|
| QA-01 | GUI acceptanceのsource保存・recovery、Debug/Release build、修正前後と現行Releaseのnative PrintWindow screenshot（`native-screenshot-current-20260921.json`） | クリック操作・主観的な文字位置は未実施 |
